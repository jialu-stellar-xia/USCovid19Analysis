...
plot_usmap(data = na.omit(covid19_tot_cases), values = "all_total", color = "#0072B2") + 
  scale_fill_continuous(name = "Confirmed (in thousands)",low = "white", high = "#0072B2") + 
...
plot_usmap(data = na.omit(covid19_new_cases), values = "all_new_total", color = "#0072B2") + 
  scale_fill_continuous(name = "Increased (in thousands)",low = "white", high = "#0072B2") + 
  theme(legend.position = "right") +
...
ggplot(data=Covid_19, aes(x=X, y = increase_rate, group = state)) + 
  geom_line() + ggtitle("New case / Total case by States") + facet_wrap(~state)
```

From this graph we can see that the 50 states have a similar pattern in the ratio of New case to Total case over time: very high at the beginning, and then be stable at a low level. It can be a signal that many confirmed patients have been recovered
, also shows  that covid-19 control measures have been effective.
...
Cdd_data<-Covid_19 %>% 
  ...
  mutate(month=cut(as.Date(date),breaks='month',labels=c('Mar',"Apr","May","Jun","Jul","Aug","Sep","Oct","Nov"))) %>% 
  group_by(month,state) %>% 
  summarise(avrg_death_rate=round(mean(death_rate),4))
```
...
pie_tidydata %>% ggplot(aes(x="",y=value,fill=group))+
  ...
  scale_fill_brewer()+
  ggtitle("Inpatient beds utilization (NJ 11/15/2020)")+
  theme(plot.title = element_text(face="bold",hjust = 0.5))
```
